...
namespace D2P0JX_HFT_2021221.Logic.Services
{
    public class PizzaLogic : IPizzaLogic
    {
        ...
        public Pizza Update(Pizza entity)
        {
            ...
            {
                throw new ArgumentException("Failed to update data. This pizza is not found in the database");
            }
        }
        public void Delete(int id)
        {
            var entity = _PizzaRepository.Read(id);
            if (entity == null ? false : entity.Customers.Any())
            {
                throw new Exception("Failed to delete this Pizza, there are customer items associated with this item.");
            }
